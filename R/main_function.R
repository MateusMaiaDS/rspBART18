...
rspBART <- function(x_train,
  ...
  if(interaction_term){
    ...
    if(is.null(interaction_list)){
      ...
      if(is.matrix(interaction_list)){
        ...
        stop("Something wrong with interaction_list")
      }
    }
  ...
  all_P_joint <- vector('list',length = length(dummy_x$continuousVars)+NCOL(interaction_list))
  ...
  for(k in 1:data$n_tree){
      if(data$all_var){
        ...
        if(k <= length(dummy_x$continuousVars)){
          forest[[k]] <- init_stump(data = data,initial_var = k)
        } else {
  ...
  for(i in 1:n_mcmc){
    ...
    for(t in 1:data$n_tree){
      ...
      if(verb == "grow"){
          ...
          forest[[t]] <- change_variable(tree = forest[[t]],
                                         curr_part_res = partial_residuals,
                                         data = data)
      }
      # Updating the betas
      update_betas_aux <- updateBetas(tree = forest[[t]],
                                      curr_part_res = partial_residuals,
                                      data = data,trees_fit = trees_fit,
                                      tree_number = t)

      # Updating the trees and the betas
      forest[[t]] <- update_betas_aux$tree
      trees_fit <- update_betas_aux$trees_fit

      if(main_effects_pred){
          tree_main_effects[[i]][,,t] <- update_betas_aux$y_hat_train
      }

      # This is for the old approach of calculating taubetas with a bigD
      if(use_D_bool){
          trees_fit[t,] <- update_betas_aux$tree_hat_train
          trees_fit_test[t,] <- update_betas_aux$tree_hat_test
      }
  ...
  if(plot_preview){
    if(interaction_term){
      ...
      for(jj in 1:length(main_effects_train_list_norm)){
        ...
        # else if(jj == 5){
        #     plot(x_train[,1], x_train[,2], cex = colMeans(main_effects_train_list_norm[[jj]][1:i,,drop = FALSE]))
        # }
        ...
        for(tree_number in 1:data$n_tree){
            ...
            if(main_effects_pred){
                for(mcmc_aux in 1:n_mcmc){
                  aux_main_effect_matrix[mcmc_aux,] <- unnormalize_bart_me(tree_main_effects[[mcmc_aux]][,jj,tree_number],a = min_y,b = max_y)
                }
            }
  ...
  if(plot_preview){
    rmse(all_y_hat_test_norm[1001:i,] %>% colMeans(na.rm = TRUE), y = sim_test$y)
